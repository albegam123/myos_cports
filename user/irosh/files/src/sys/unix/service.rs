//! Unix implementation of service management (dinit, systemd and launchd).
...
///
/// On Linux this checks dinit first and otherwise systemd; on macOS it checks
/// launchd.
#[must_use]
...
async fn query_status_linux(_state: Option<PathBuf>) -> ServiceStatus {
    if Path::new("/usr/bin/dinitctl").is_file() && Path::new("/usr/lib/dinit.d/irosh").is_file() {
        let output = tokio::task::spawn_blocking(|| {
            std::process::Command::new("/usr/bin/dinitctl")
                .args(["is-started", "irosh"])
                .output()
        })
        .await;
        return match output {
            Ok(Ok(out)) if out.status.success() => ServiceStatus::Active("dinit".to_string()),
            Ok(Ok(_)) => ServiceStatus::Inactive,
...
///
/// Delegates to platform-specific implementations (dinit/systemd or launchd).
///
/// # Errors
///
...
async fn handle_service_linux(action: ServiceAction, state: Option<PathBuf>) -> Result<()> {
    if Path::new("/usr/bin/dinitctl").is_file() && Path::new("/usr/lib/dinit.d/irosh").is_file() {
        let dinit_action = match action {
            ServiceAction::Install | ServiceAction::Start => "start",
            ServiceAction::Uninstall | ServiceAction::Stop => "stop",
        };
        let status = tokio::task::spawn_blocking(move || {
            std::process::Command::new("/usr/bin/dinitctl")
                .args([dinit_action, "irosh"])
                .status()
        })
        .await
        .map_err(|e| ServerError::ServiceManagement {
            details: format!("dinitctl {dinit_action} failed: {e}"),
        })?
        .map_err(|e| ServerError::ServiceManagement {
            details: format!("dinitctl {dinit_action} failed: {e}"),
        })?;
        if !status.success() {
            return Err(ServerError::ServiceManagement {
                details: format!("dinitctl {dinit_action} exited with {status}"),
            }
            .into());
        }
        return Ok(());
